...
/* Setup dataTable AJAX when document is ready*/
...
$("body").on("click", "#edit_category,#create_new_category", function () {
	...
	if (modal_type == 'create_new_category') {
		...
		$("#categoryModal").modal({
		    ...
		    /*
		    // Get detail record form table
		    $("#category_id").val($(this).data("id"));
		    $("#name").val($(this).data("name"));
		    $("#categoryModalTitle").html("Edit Category");
		    $("#btn_save").html("Update");
		    $("#categoryModal").modal({show: true,backdrop: 'static', keyboard: false});
		    */
...
$("#btn_ok").click(function () {
	...
	switch (url_delete) {
		case 'permanent_delete_category':
			url_action = '/hardDelete';
			break;
		case 'restore_category':
			url_action = '/restoreDelete';
			url_type = "PATCH";
			msg = "Restoring...";
			break;
		default:
	...
	$.ajax({
		type: url_type,
		url: SITEURL + "/categories/" + category_id + url_action,
		beforeSend: function () {
			$("#btn_ok").text(msg);
		},
