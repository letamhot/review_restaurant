@extends('front-end.layout.web')
@push('css')
<link rel="stylesheet" href="{{  asset('assets\backend\modules\izitoast\css\iziToast.min.css') }}">
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css" />
<link href="//cdnjs.cloudflare.com/ajax/libs/semantic-ui/2.2.2/components/icon.min.css" rel="stylesheet">
<link href="//cdnjs.cloudflare.com/ajax/libs/semantic-ui/2.2.2/components/comment.min.css" rel="stylesheet">
<link href="//cdnjs.cloudflare.com/ajax/libs/semantic-ui/2.2.2/components/form.min.css" rel="stylesheet">
<link href="//cdnjs.cloudflare.com/ajax/libs/semantic-ui/2.2.2/components/button.min.css" rel="stylesheet">
<link href="{{ asset('/vendor/laravelLikeComment/css/style.css') }}" rel="stylesheet">
@endpush

<body>
    <header>
        @include('front-end.partials.nav')
    </header>
    @section('content')
    <main class="main-container-detail">
        <aside class="left-side-detail">
            <div class="menu">
                @guest
                    <a style="text-decoration:none" href="javascript:void(0)" id="guest_like">
                        <i class="fa fa-heart fa-2x" style="color: red; padding:10px"></i>
                        <span class="badge badge-white">
                            {{ $post_detail->totalLike + $post_detail->totalStar }}
                        </span>
                    </a> ​
                    <br>
                    <a style="text-decoration:none" href="javascript:void(0)" id="guest_star">
                        <i style="color: green; padding:14px" class="fa fa-bookmark fa-2x"></i>
                        <span class="badge badge-white">
                            {{ $post_detail->totalStar }}
                        </span>
                    </a>
                    <br>
                    <a href="javascript:void(0)" style="text-decoration:none">
                        <i title="Lượt bình luận" id="guest_comment" style="color: blue;padding:10px "
                            class="fa fa-comments fa-2x">
                        </i>
                        <span class="badge badge-white">{{ $post_detail->totalComment }}</span>
                    </a>
                    <br> ​
                @else
                    <a style="text-decoration:none" href="javascript:void(0)" data-id='{{ $post_detail->id }}'
                        data-name="like" id="user_like">
                        <i title="yêu thích"
                            class="{{ $post_detail->isUserLiked == false ? 'fa fa-heart fa-2x'  : 'fa fa-heart-o fa-2x' }}"
                            style="color: red; padding:10px ">
                        </i>
                        <span class="badge badge-white" id="like_count">
                            {{ $post_detail->totalLike }}
                        </span>
                    </a>
                    <br> ​
                    <a style="text-decoration:none" href="javascript:void(0)" data-id='{{ $post_detail->id }}'
                        data-name="star" id="user_star">
                        <i class=" {{ $post_detail->isUserFavorited == false ? 'fa fa-bookmark fa-2x' : 'fa fa-bookmark-o fa-2x' }}"
                            title="Lưu lại" style="color: green; padding:14px">
                        </i>
                        <span class="badge" id="bookmark_count">
                            {{ $post_detail->totalStar }}
                        </span>
                    </a>
                    <br>
                    <a href="#comment_place" style="text-decoration:none">
                        <i title="Lượt bình luận" style="color: blue;padding:10px"
                            class="fa fa-comments fa-2x">
                        </i>
                        <span class="badge badge-white">{{ $post_detail->totalComment }}</span>
                    </a>
                    <br>
                @endguest
                <div id="social-links">
                    <a style="padding-left:10px"
                        href="https://www.facebook.com/sharer/sharer.php?u={{ url()->current() }}"
                        class="social-button">
                        <i title="chia sẻ facebook" class="fa fa-share-alt fa-2x"></i>
                    </a>
                </div>
            </div>

        </aside>

        <div class="main-content-detail">
            <div class="tweet-card">
                <div class="tweet-card__content">
                    <div class="tweet-card__thumbnail">
                        <img
                            src="data:image/jpeg;base64," />
                    </div>

                    <h1 class="tweet-card__title">{{ $post_detail->title }}</h1>
                    <div class="tweet-card__header">
                        <div class="tweet-card__avatar-wrapper">
                            <img src="{{ $post_detail->user->avatar }}" />
                        </div>

                        <div class="tweet-card__info">
                            <a class="tweet-card__user-name">{{ $post_detail->user->name }}</a>
                            <span class=" tweet-card__date-post">{{ $post_detail->created_at }}</span>
                        </div>
                    </div>


                    <div class="tweet-card__tags-container">
                        @foreach ($post_detail->tag as $value)

                        <a href="{{ route('showdetailtag', $value->id) }}" style="color: blue; font-weight: bolder;">
                            #{{ $value->name }}
                        </a> @endforeach
                    </div>
                    <div>
                        {!! $post_detail->content !!}
                    </div>

                    <div class="tweet-card__actions-container">
                        <div id="comment_place">

                            <button class="linh-button button-md">
                                <i class="fa fa-bell-o" aria-hidden="true"></i>
                                {{ $post_detail->totalLike + $post_detail->totalStar }} tương tác
                            </button>
                            <button class="linh-button button-md">
                                <i class="fa fa-bell-o" aria-hidden="true"></i>
                                {{ $post_detail->totalComment }} bình luận
                            </button>
                        </div>
                    </div>
                </div>
                <div>
                    @include('laravelLikeComment::comment', ['comment_item_id' => $post_detail->id])
                </div>
            </div>
        </div>
        </div>
        </div>


        <aside class="right-side-detail">
            <div class="hot-user">
                <div class="card">
                    <img src="{{ $post_detail->user->avatar }}" alt="John" style="width:100%; height:150px">
                    <h3>{{ $post_detail->user->name }}</h3>
                    <p class="title">{{ $post_detail->user->about }}</p>
                    <div style="margin: 24px 0;">
                        <a href="#"><i class="fa fa-dribbble"></i></a>
                        <a href="#"><i class="fa fa-twitter"></i></a>
                        <a href="#"><i class="fa fa-linkedin"></i></a>
                        <a href="#"><i class="fa fa-facebook"></i></a>
                    </div>
                    <p><button>Liên Hệ</button></p>
                </div>
            </div>
        </aside>

    </main>

    <script src="https://code.jquery.com/jquery-3.5.1.js"
        integrity="sha256-QWo7LDvxbWT2tbbQ97B53yJnYU3WhH/C8ycbRAkjPDc=" crossorigin="anonymous">
    </script>

    <script src="{{ asset('assets/backend/modules/izitoast/js/iziToast.min.js')}}"></script>

    <script>
        var token = '{!! csrf_token() !!}';
    </script>
    <script src="{{ asset('js/ajax_reaction.js')}}"></script>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.0/jquery.min.js"></script>
    <script src="{{ asset('/vendor/laravelLikeComment/js/script.js') }}" type="text/javascript"></script>
    @endsection