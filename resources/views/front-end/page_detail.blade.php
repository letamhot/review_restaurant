@extends('front-end.layout.web')
@push('detail-css')
<link href="//cdnjs.cloudflare.com/ajax/libs/semantic-ui/2.2.2/components/icon.min.css" rel="stylesheet">
<link href="//cdnjs.cloudflare.com/ajax/libs/semantic-ui/2.2.2/components/comment.min.css" rel="stylesheet">
<link href="//cdnjs.cloudflare.com/ajax/libs/semantic-ui/2.2.2/components/form.min.css" rel="stylesheet">
<link href="//cdnjs.cloudflare.com/ajax/libs/semantic-ui/2.2.2/components/button.min.css" rel="stylesheet">
<link href="{{ asset('/vendor/laravelLikeComment/css/style.css') }}" rel="stylesheet">
@endpush
<body>
    <header>
        @include('front-end.partials.nav')
    </header>
    @section('content')
    <main class="main-container-detail">
        <aside class="left-side-detail">
            <div class="menu">
                <a href="javascript:;" class="linh-nav-icon-button"><i title="Like" onclick="myFunction(this)"
                        style="color: blue" class="fa fa-thumbs-up" aria-hidden="true"></i></a>
                <br>
                <a href="javascript:;" class="linh-nav-icon-button"><i title="Lượt thích" class="fa fa-heart "
                        style="color: red" aria-hidden="true"></i></a><span>1567</span>
                <br>
                <a href="javascript:;" class="linh-nav-icon-button"><i title="Lượt bình luận"
                        class="fa fa-comments"></i></a><span>5000</span>
            </div>
        </aside>

        <div class="main-content">
            <div class="tweet-card">
                <div class="tweet-card__header">
                    <div class="tweet-card__avatar-wrapper">
                        <img
                            src="data:image/jpeg;base64," />
                    </div>

                    <div class="tweet-card__info">
                        <a class="tweet-card__user-name">Elon musk</a>
                        <span class="tweet-card__date-post">May 2 '20</span>
                    </div>
                </div>
                <div class="tweet-card__content">
                    <div class="tweet-card__thumbnail">
                        <img
                            src="data:image/jpeg;base64," />
                    </div>

                    <a href="" class="tweet-card__title">9 Projects you can do to become a Frontend Master
                        in 2020</a>

                    <div class="tweet-card__tags-container">
                        <a href="" style="color: blue; font-weight: bolder;">#career</a>
                        <a href="" style="color: red;font-weight: bolder;">#beginner</a>
                        <a href="" style="color: orange;font-weight: bolder;">#sequel</a>
                        <a href="" style="color: green;font-weight: bolder;">#python</a>
                    </div>
                    <div>
                        nội dung bài post Lorem ipsum dolor sit amet,
                        consectetur adipiscing elit, sed do eiusmod
                        tempor incididunt ut labore et dolore magna
                        aliqua. Ut enim ad minim veniam, quis nostrud
                        exercitation ullamco laboris nisi ut aliquip ex
                        ea commodo consequat. Duis aute irure dolor in
                        reprehenderit in voluptate velit esse cillum
                        dolore eu fugiat nulla pariatur. Excepteur sint
                        occaecat cupidatat non proident, sunt in culpa
                        qui officia deserunt mollit anim id est laborum.
                        Lorem ipsum dolor sit amet, consectetur
                        adipiscing elit, sed do eiusmod tempor
                        incididunt ut labore et dolore magna aliqua. Ut
                        enim ad minim veniam, quis nostrud exercitation
                        ullamco laboris nisi ut aliquip ex ea commodo
                        consequat. Duis aute irure dolor in
                        reprehenderit in voluptate velit esse cillum
                        dolore eu fugiat nulla pariatur. Excepteur sint
                        occaecat cupidatat non proident, sunt in culpa
                        qui officia deserunt mollit anim id est laborum.
                        Lorem ipsum dolor sit amet, consectetur
                        adipiscing elit, sed do eiusmod tempor
                        incididunt ut labore et dolore magna aliqua. Ut
                        enim ad minim veniam, quis nostrud exercitation
                        ullamco laboris nisi ut aliquip ex ea commodo
                        consequat. Duis aute irure dolor in
                        reprehenderit in voluptate velit esse cillum
                        dolore eu fugiat nulla pariatur. Excepteur sint
                        occaecat cupidatat non proident, sunt in culpa
                        qui officia deserunt mollit anim id est laborum.
                    </div>

                    <div class="tweet-card__actions-container">
                        <div>
                            <span class="tweet-card__read-count">
                                13 min read
                            </span>

                            <button class="linh-button button-md button-silver">
                                Save
                            </button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="tweet-card">
                <form>
                    <textarea>Viết Bình Luận...</textarea>
                    <br>
                    <input class="linh-button button-lg button-success" type="submit" value="Gửi Bình Luận">
                </form>
            </div>

            @include('laravelLikeComment::comment', ['comment_item_id' => 'video_12'])

            <div class="tweet-card">
                <div class="main-content-comment">
                    <button id="hide">Hide</button>
                    <button id="show">Show</button>
                    <br>
                    <h2>Bình Luận</h2>
                    <hr />
                    <div class="tweet-card__header">
                        <div class="comment__avatar-wrapper">
                            <img
                                src="data:image/jpeg;base64," />
                        </div>
                        <div class="comment__info">
                            <p style="font-size: small; font-weight: bold;">
                                Chỉ Số Đánh Giá: 10/10
                            </p>
                            <a class="tweet-card__user-name">Nhật Linh Phan</a>
                            <span class="tweet-card__date-post">21 May 2019</span>
                        </div>
                        <div style="display: block;">
                            <p>
                                The meals were simple but really delicious and
                                innovative. We really enjoyed the cuisine. Wine list
                                was really disappointing. There are a few items from
                                Chalonnaise and that is it. We expected more.
                            </p>
                        </div>
                    </div>
                    <hr />
                    <div class="tweet-card__header">
                        <div class="comment__avatar-wrapper">
                            <img
                                src="data:image/jpeg;base64," />
                        </div>
                        <div class="comment__info">
                            <p style="font-size: small; font-weight: bold;">
                                Chỉ Số Đánh Giá: 10/10
                            </p>
                            <a class="tweet-card__user-name">Nhật Linh Phan</a>
                            <span class="tweet-card__date-post">21 May 2019</span>
                        </div>
                        <div style="display: block;">
                            <p>
                                The meals were simple but really delicious and
                                innovative. We really enjoyed the cuisine. Wine list
                                was really disappointing. There are a few items from
                                Chalonnaise and that is it. We expected more.
                            </p>
                        </div>
                    </div>
                    <hr />
                    <div id="test" class="tweet-card__header">
                        <div class="comment__avatar-wrapper">
                            <img
                                src="data:image/jpeg;base64," />
                        </div>
                        <div class="comment__info">
                            <p style="font-size: small; font-weight: bold;">
                                Chỉ Số Đánh Giá: 10/10
                            </p>
                            <a class="tweet-card__user-name">Nhật Linh Phan</a>
                            <span class="tweet-card__date-post">21 May 2019</span>
                        </div>
                        <div style="display: block;">
                            <p>
                                The meals were simple but really delicious and
                                innovative. We really enjoyed the cuisine. Wine list
                                was really disappointing. There are a few items from
                                Chalonnaise and that is it. We expected more.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        </div>


        <aside class="right-side-detail">
            <div class="hot-tweet-list">
                <h3>Thông Tin user</h3>
            </div>

            <div class="hot-user">
                <h1 class="hot-tweet-list__title">Tên của User</h1>
                <p>Thông tin cá nhân của User + Hình ảnh</p>
                {{-- <ul>
                    <li>
                        Tuổi:
                    </li>
                    <li>
                        nơi sinh sống
                    </li>
                    <li>
                        Công việc
                    </li>
                </ul> --}}
            </div>
        </aside>
    </main>
@push('detail-js')
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.0/jquery.min.js"></script>
<script src="{{ asset('/vendor/laravelLikeComment/js/script.js') }}" type="text/javascript"></script>
@endpush
    <script src="https://code.jquery.com/jquery-3.5.1.js"
        integrity="sha256-QWo7LDvxbWT2tbbQ97B53yJnYU3WhH/C8ycbRAkjPDc=" crossorigin="anonymous">
    </script>
    <script>
        function myFunction(x) {
        x.classList.toggle("fa-thumbs-down");
        }

        $(document).ready(function(){
        $("#hide").click(function(){
            $("#test").hide();
        });
        $("#show").click(function(){
            $("#test").show();
        });
        });
    </script>
    @endsection

