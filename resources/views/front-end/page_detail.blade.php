@extends('front-end.layout.web')

<body>
    <header>
        @include('front-end.partials.nav')
    </header>
    @section('content')
    <main class="main-container-detail">
        <aside class="left-side-detail">
            <div class="menu">
                @if(Auth::user())
                {{-- <a href="javascript:;" class="linh-nav-icon-button"><i title="Like" onclick="myFunction(this)"
                        style="color: blue" class="fa fa-thumbs-up" aria-hidden="true"></i></a> --}}
                <br>
                <a href="#" class="linh-nav-icon-button" id="show_hide_bt"><i title="Yêu thích" class="fa fa-heart-o "
                        style="color: red" aria-hidden="true"></i></a><span>1567</span>
                <br>
                <a href="#" class="linh-nav-icon-button" id="show_hide"><i title="Đánh Dấu" class="fa fa-bookmark-o "
                        style="color:purple" aria-hidden="true"></i></a>
                <br>
                <a href="#comment" class="linh-nav-icon-button"><i title="Bình luận" style="color: blue"
                        class="fa fa-comments"></i></a><span>5000</span>
                @else
                <br>
                <a href="/login" class="linh-nav-icon-button"><i title="Lượt thích" class="fa fa-heart-o "
                        style="color: red" aria-hidden="true"></i></a><span>1567</span>
                <br>
                <a href="/login" class="linh-nav-icon-button" id="show_hide"><i title="Đánh Dấu"
                        class="fa fa-bookmark-o " style="color:purple" aria-hidden="true"></i></a>
                <br>
                <a href="/login" class="linh-nav-icon-button"><i title="Lượt bình luận" style="color: blue"
                        class="fa fa-comments"></i></a><span>5000</span>

                @endif
            </div>
        </aside>

        <div class="main-content-detail">
            <div class="tweet-card">
                {{-- <div class="tweet-card__header">
                    <div class="tweet-card__avatar-wrapper">
                        <img
                            src="data:image/jpeg;base64," />
                    </div>

                    <div class="tweet-card__info">
                        <a class="tweet-card__user-name">Elon musk</a>
                        <span class="tweet-card__date-post">May 2 '20</span>
                    </div>
                </div> --}}
                <div class="tweet-card__content">
                    <div class="tweet-card__thumbnail">
                        <img
                            src="data:image/jpeg;base64," />
                    </div>

                    <h1 class="tweet-card__title">{{ $post_detail->title }}</h1>
                    <div class="tweet-card__header">
                        <div class="tweet-card__avatar-wrapper">
                            <img src="{{ $post_detail->user->avatar }}" />
                        </div>

                        <div class="tweet-card__info">
                            <a class="tweet-card__user-name">{{ $post_detail->user->name }}</a>
                            <span class=" tweet-card__date-post">May 2 '20</span>
                        </div>
                    </div>

                    <div class="tweet-card__tags-container">
                        @foreach ($post_detail->tag as $value)

                        <a href="{{ route('showdetailtag', $value->id) }}" style="color: blue; font-weight: bolder;">
                            #{{ $value->name }}
                        </a>

                        @endforeach
                    </div>
                    <div>
                        {!! $post_detail->content !!}
                    </div>

                    <div class="tweet-card__actions-container">
                        <div>
                            <span class="tweet-card__read-count">
                                13 min read
                            </span>

                            <button class="linh-button button-md button-silver">
                                Save
                            </button>
                        </div>
                    </div>
                </div>
            </div>
            <div id="comment" class="tweet-card">
                <form>
                    <textarea>Viết Bình Luận...</textarea>
                    <br>
                    <input class="linh-button button-lg button-success" type="submit" value="Gửi Bình Luận">
                </form>
            </div>
            <div class="tweet-card">
                <div class="main-content-comment">
                    <button id="hide">Hide</button>
                    <button id="show">Show</button>
                    <br>
                    <h2>Bình Luận</h2>
                    <hr />
                    <div class="tweet-card__header">
                        <div class="comment__avatar-wrapper">
                            <img
                                src="data:image/jpeg;base64," />
                        </div>
                        <div class="comment__info">
                            <p style="font-size: small; font-weight: bold;">
                                Chỉ Số Đánh Giá: 10/10
                            </p>
                            <a class="tweet-card__user-name">Nhật Linh Phan</a>
                            <span class="tweet-card__date-post">21 May 2019</span>
                        </div>
                        <div style="display: block;">
                            <p>
                                The meals were simple but really delicious and
                                innovative. We really enjoyed the cuisine. Wine list
                                was really disappointing. There are a few items from
                                Chalonnaise and that is it. We expected more.
                            </p>
                        </div>
                    </div>
                    <hr />
                    <div class="tweet-card__header">
                        <div class="comment__avatar-wrapper">
                            <img
                                src="data:image/jpeg;base64," />
                        </div>
                        <div class="comment__info">
                            <p style="font-size: small; font-weight: bold;">
                                Chỉ Số Đánh Giá: 10/10
                            </p>
                            <a class="tweet-card__user-name">Nhật Linh Phan</a>
                            <span class="tweet-card__date-post">21 May 2019</span>
                        </div>
                        <div style="display: block;">
                            <p>
                                The meals were simple but really delicious and
                                innovative. We really enjoyed the cuisine. Wine list
                                was really disappointing. There are a few items from
                                Chalonnaise and that is it. We expected more.
                            </p>
                        </div>
                    </div>
                    <hr />
                    <div id="test" class="tweet-card__header">
                        <div class="comment__avatar-wrapper">
                            <img
                                src="data:image/jpeg;base64," />
                        </div>
                        <div class="comment__info">
                            <p style="font-size: small; font-weight: bold;">
                                Chỉ Số Đánh Giá: 10/10
                            </p>
                            <a class="tweet-card__user-name">Nhật Linh Phan</a>
                            <span class="tweet-card__date-post">21 May 2019</span>
                        </div>
                        <div style="display: block;">
                            <p>
                                The meals were simple but really delicious and
                                innovative. We really enjoyed the cuisine. Wine list
                                was really disappointing. There are a few items from
                                Chalonnaise and that is it. We expected more.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        </div>


        <aside class="right-side-detail">
            <div class="hot-user">
                <div class="tweet-card__header">
                    <div class="tweet-card__avatar-wrapper">
                        <img src="{{ $post_detail->user->avatar }}" />
                    </div>

                    <div class="tweet-card__info">
                        <a class="tweet-card__user-name">{{ $post_detail->user->name }}</a>
                        <span class=" tweet-card__date-post">{{ $post_detail->user->email }}</span>

                    </div>
                </div>

                <div class="tweet-card__tags-container">
                    <p style="font-style: italic; font-weight:bolder">{{ $post_detail->user->about }} </p>
                </div>
            </div>
        </aside>

    </main>

    <script src="https://code.jquery.com/jquery-3.5.1.js"
        integrity="sha256-QWo7LDvxbWT2tbbQ97B53yJnYU3WhH/C8ycbRAkjPDc=" crossorigin="anonymous">

    </script>
    <script>
        $("#show_hide_bt").click(function(event) {
            $(this).find('i').toggleClass('fa fa-heart-o').toggleClass('fa fa-heart');
        });
        $("#show_hide").click(function(event) {
            $(this).find('i').toggleClass('fa fa-bookmark-o').toggleClass('fa fa-bookmark');
        });
        $(document).ready(function(){
        $("#hide").click(function(){
            $("#test").hide();
        });
        $("#show").click(function(){
            $("#test").show();
        });
        });
    </script>
    @endsection
